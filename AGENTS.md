...
- 2026-07-16：`DailOverview.vue` 文案与排版改版。文案改为「不做交付即终点的项目。/ 做能自我进化的行业智能。」＋右上说明（四层全栈自研），中英双语在 `stores/i18n.js` 的 `dailOverview` 下维护。排版从原「构建/治理/落地＋理念句」错落四区，改为**编号关键点形式**：保留左上两行大标题（第二行字重略重）与右上说明，下方是「全栈四层」四个关键点（`01 数据 / 02 模型 / 03 智能体 / 04 平台`，每点为顶部细线＋编号＋关键词＋一句说明，桌面 4 列、`900px` 以下 2 列），理念句收在右下（窄屏改左对齐）；关键点在 i18n 中以 `point{n}Key/point{n}Desc` 扁平键维护，序号由组件按索引生成。因此**本区块已不再遵守 2026-07-15 记录里“不显示编号/小标签/分隔线”的旧约定**。进场动画走对角节奏：标题残影揭字 → 右上说明遮罩上移 → 四个关键点按序「细线从左展开＋编号/关键词/说明错峰上移」→ 理念句收尾；仍保留标题行 `overflow:hidden` 揭字（已把标题行 `padding-bottom` 加到 `0.2em` 防止英文下伸笔画被裁），以及回到首页顶部复位重播、`900px` 移动布局和减少动画降级。
- 2026-07-16：`DailOverview.vue` 再次改版为「64 开布局＋关键点 hover 图文切换」。桌面网格从原全宽错落改为 6:4 感的两区：左区（约 7/12 列）放两行大标题与四个关键点，右区（约 5/12 列，`grid-column: 8 / -1`）上方是固定 `3:2` 比例的展示框、下方是理念句；标题字号上限从 94px 降到 88px 以适配变窄的左区。右上展示框默认显示 `intro` 说明文字，鼠标移到/键盘 focus 某个关键点时，说明克制淡出上移，同位置淡入该关键点对应的一张展示图（`scale(1.03)→1`＋去模糊＋淡入），移开恢复文字；四张占位图在 `frontend/src/assets/images/overview/`（`data/models/agents/platform.svg`，`3:2`，待真图替换），组件按 `pointImages` 顺序绑定，激活项以关键点顶部黑色细线 `scaleX` 展开＋其余项 `opacity:0.4` 变暗作反馈。交互仅在桌面（`matchMedia('(min-width:901px)')`）启用，`activePoint` 记录当前项；`900px` 以下展示框 `aspect-ratio:auto`、图片 `display:none`、`intro` 回到自然流，与之前移动端阅读流一致。展示框比例是唯一可调点（改 `.overview-display__frame` 的 `aspect-ratio`，可在 `3:2 / 4:3 / 16:10` 间切换）。保留进场动画、回顶复位重播与减少动画降级。
- 2026-07-17：`DailOverview.vue` 的四张 hover 占位 SVG 已替换为正式 PNG：`data.png` 对应数据、`models.png` 对应模型、`agents.png` 对应智能体、`platform.png` 对应平台；图片原始尺寸统一为 `1672×941`，展示框同步改为 `16:9`，继续使用 `background-size:cover`，避免原 `3:2` 展示框裁切左右内容。桌面 hover/键盘 focus 动画和 `900px` 以下不显示图片的规则保持不变；本条覆盖上一条记录中的占位图与 `3:2` 比例说明。
- 2026-07-17：`DailOverview.vue` 的四个关键点在桌面端增加自动轮播：按 `01 数据 → 02 模型 → 03 智能体 → 04 平台` 循环，完全沿用首页 `Main.vue` 首屏的 `4000ms` 播放时长和 `40ms` 更新步长；关键点顶部改为与首屏一致的 `2px` 浅灰底线＋深色实时进度填充，数字同步为 `14px`、`0.18em` 字距和相同的激活/未激活颜色。鼠标移入或键盘 focus 某项时停止定时器、立即显示该项图片并从零开始，移出或失焦后从当前项重新计时；`900px` 以下继续只显示说明文字和 2×2 关键点，不自动切图，减少动画模式固定显示第一项且不自动轮播。
...
- 2026-07-17：首页在 `AIOSShowcase.vue` 与 `Third.vue` 之间新增 `WibebareShowcase.vue`，文案接入现有 `stores/i18n.js`，样式通过组件级 scoped 外部 CSS 隔离。当前只保留首屏到 `wibebare-dashboard.png` 放大铺满的滚动卡点：“开始免费试用”按钮已移除，说明和流程条退场后产品图放大，完成后继续下滚会平滑缩小、虚化并淡出，再释放到原 `Third.vue` CTA；原左侧缩略图功能说明、工具轮播、管理页、成效页及其后续交接均已移除。AIOS 与 Wibebare 现在使用重叠交接：桌面端 AIOS 青白虚化进度约 75% 时 Wibebare 才从雾光上方同步淡入，白色覆盖完成时产品首屏就位，不再停留纯白空屏；手机端保留缩短后的静态渐变尾部，并在渐变后段提前淡入 Wibebare。Wibebare 滚动更新已改为每帧合并，不再使用 50ms 轮询；向下仅保留一次短时放大吸附，向上完全跟随原生滚动连续还原，不再强制吸回卡点。
- 2026-07-18：`AIOSShowcase.vue` 桌面端向下交接改为固定 `1200ms` 播放：AIOS 场景进入顶部交互区后，滚轮下滑、`ArrowDown`/`PageDown`/`End`/空格或单指上划中的一次前进输入会启动完整青白扩散、内容退场及 Wibebare 重叠淡入，并同步把页面平滑推进到 AIOS sticky 段末端；动画结束前持续拦截后续前进输入，完成后才释放自然下滑。`900px` 以下和减少动画模式仍保持原有自然滚动降级。
- 2026-07-18：`WibebareShowcase.css` 的滚动轨道由 `308svh` 缩短为 `150svh`，同样的滚轮位移会推进更多标题覆盖、内容退场和产品图放大进度；原有向下卡点与向上原生连续还原逻辑保持不变。
- 2026-07-21：首页滚动交互统一改为原生连续推进：`TechnologyNemoTransition.vue` 保留约 `980ms` 的技术/Nemo 视觉交接，但达到 sticky 场景阈值后自动触发且不再拦截滚轮、键盘或触摸输入；`AIOSShowcase.vue` 取消固定 `1200ms` 强制滚动到段末，恢复约 `105ms` 惯性跟随的滚动擦除；`WibebareShowcase.vue` 取消向下自动吸附，标题退场、产品图放大与后续淡出均只按实际滚动位置连续变化。三段保留原视觉顺序、`900px` 移动端降级和减少动画模式，但页面不再出现连续的滚动锁定。
- 2026-07-23：`WibebareShowcase.vue` 桌面端改为“文字分段定时、产品图跟随滚动”的组合逻辑：进入对应阈值后依次完整播放说明/流程退场和大标题退场，产品图在文字完成前最多到预备缩放，不会提前进入铺满放大；仅在文字与图片交接点设置窄门槛，文字结束后继续下滑才由滚动控制图片放大。图片退场仍跟随滚动，并在完全缩回、虚化和淡出前保留最终出口门槛；向上滚动始终允许并可反向恢复，其他区间不锁定页面滚动。
